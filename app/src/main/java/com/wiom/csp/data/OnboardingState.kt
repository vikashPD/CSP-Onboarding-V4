...
/** API contract — will move to Retrofit layer when backend is ready */
...
object OnboardingState {
    ...
    // Dashboard-driven navigation target (observed by NavGraph)
    var dashboardNavTarget by mutableIntStateOf(-1)
    ...
    fun goTo(index: Int) {
        if (index in 0 until TOTAL_SCREENS) {
            currentScreen = index
            dashboardNavTarget = index
        }
    }
