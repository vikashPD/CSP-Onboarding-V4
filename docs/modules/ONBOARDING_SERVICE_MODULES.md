...
│  └──┬───┘   └──────┘   └──────┘   └────┬─────┘                         │
│     │ ✗ Duplicate → END                 │ ✗ Failed/Timeout → Retry      │
...
│                             ▼                   ▼                       │
│                        [Approved]          [Rejected]                   │
│                             │              → Refund via PG (M4) → END   │
│                             ▼                                           │
│                    ┌────────┴─────────┐                                  │
│                    ▼                  ▼                                  │
│              [Tech Passed]     [Tech Rejected]                          │
│                    │           → No refund → END                        │
│                    ▼                                                    │
│  ┌──────┐   ┌──────────┐   ┌──────┐                                     │
│  │  M6  │──▶│    M4    │──▶│  M7  │                                     │
...
**Key points:**
- **M1** can end the journey on duplicate identity
- **M4 is invoked twice** — registration fee (Phase 1) and onboarding fee (Phase 3); both can fail/timeout with retry
- **M5 has two branch points** — QA review (approved/rejected) + technical assessment (passed/rejected)
- **M5 → M4 reverse flow** — verification rejection signals M4 to coordinate refund via PG
- **M5 tech rejected** — no refund, journey ends
- **M7** can fail or pend with retry
...
PARTNER OPENS APP
  ...
  │         │     └── APPROVED → Tech Assessment (sample device telemetry)
